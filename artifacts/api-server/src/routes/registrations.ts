...
import { publicRegistrationRateLimiter } from "../lib/rateLimits";
...
function findSubmittedValue(
  ...
  const normalizedTargets = keysOrLabels.map((target) =>
  ...
  for (const field of formFields) {
    ...
    if (
      !normalizedTargets.includes(fieldKey ?? "") &&
      !normalizedTargets.includes(fieldLabel)
    ) {
      continue;
    }
...
const SYSTEM_KEY_MAP: Record<string, { table: FieldTable; column: string }> = {
  ...
  secondary_guardian_first_name: {
    table: "registrations",
    column: "secondary_guardian_first_name",
  },
  secondary_guardian_last_name: {
    table: "registrations",
    column: "secondary_guardian_last_name",
  },
  secondary_guardian_phone: {
    table: "registrations",
    column: "secondary_guardian_phone",
  },
  secondary_guardian_email: {
    table: "registrations",
    column: "secondary_guardian_email",
  },
  secondary_guardian_relationship: {
    table: "registrations",
    column: "secondary_guardian_relationship",
  },
...
router.post(
  ...
  async (req, res) => {
    ...
    const incomingGroupId =
      (req.body as { registrationGroupId?: number }).registrationGroupId ??
      null;
    ...
    try {
      ...
      const organizationId =
        form.organizationId ?? event?.organizationId ?? null;
      ...
      for (const { fieldId, value } of submittedFields) {
        ...
        if (!formField) {
          res
            ...
            .json({
              error: "Invalid input",
              details: `Field ${fieldId} does not belong to this form`,
            });
          return;
        }
      ...
      const secondaryGuardianFields = formFields.filter(
        (field) =>
      ...
      const allowSecondGuardian =
        form.allowSecondGuardian ?? secondaryGuardianFields.length > 0;
      ...
      for (const { fieldId, value } of submittedFields) {
        ...
        if (formField.fieldKind === "system" && formField.systemKey) {
          ...
          if (mapping) {
            if (mapping.table === "participants")
              participantCols[mapping.column] = value;
            else if (mapping.table === "guardians")
              guardianCols[mapping.column] = value;
            else if (mapping.table === "emergency_contacts")
              emergencyCols[mapping.column] = value;
            else if (mapping.table === "registrations")
              registrationCols[mapping.column] = value;
          } else {
            // System key with no structured column — save as custom answer so data isn't lost
            customAnswers.push({
              formFieldId: fieldId,
              questionLabel: formField.label,
              answerValue: value,
            });
          }
        } else {
      ...
      const primaryContactName =
        ...
        null;
      const participantName = compactName(
        participantCols["first_name"],
        participantCols["last_name"],
      );
      const participantNamesForEmail = confirmationParticipantNames
        .map((name) => name.trim())
        .filter(Boolean);
      ...
      if (form.confirmationEmailEnabled && !suppressConfirmationEmail) {
        try {
          await sendRegistrationConfirmationEmail({
            ...
            primaryContactName,
            participantNames:
              participantNamesForEmail.length > 0
      ...
      if (err instanceof InvalidRegistrationGroupError) {
        res
          .status(400)
          .json({
            error: "Registration group does not belong to this form and event",
          });
        return;
      }
  ...
  try {
    ...
    const [form] = await db
      ...
      .where(
        and(
          eq(formsTable.id, formId),
          eq(formsTable.organizationId, auth.organizationId),
        ),
      )
      .limit(1);
    if (!form) {
    ...
    const rows = await db
      .select({
        ...
        secondaryGuardianFirstName:
          registrationsTable.secondaryGuardianFirstName,
        ...
        secondaryGuardianRelationship:
          registrationsTable.secondaryGuardianRelationship,
        ...
        emergencyContactRelationship:
          registrationsTable.emergencyContactRelationship,
      ...
      .leftJoin(
        participantsTable,
        eq(registrationsTable.participantId, participantsTable.id),
      )
      .leftJoin(
        guardiansTable,
        eq(registrationsTable.guardianId, guardiansTable.id),
      )
      .leftJoin(
        emergencyContactsTable,
        eq(
          emergencyContactsTable.participantId,
          registrationsTable.participantId,
        ),
      )
      .where(
        and(
    ...
    const registrations = rows.map((row) => {
      ...
      const guardianName =
        row.guardianName ||
        [row.guardianFirstNameJoined, row.guardianLastNameJoined]
          .filter(Boolean)
          .join(" ");
      const guardianPhone = row.guardianPhone || row.guardianPhoneJoined || "";
      const guardianEmail =
        row.guardianEmail || row.guardianEmailJoined || null;
      return {
        id: row.id,
        formId: row.formId,
        eventId: row.eventId,
        ...
        emergencyContactRelationship:
          row.emergencyContactRelationship ?? row.ecRelationship ?? null,
        room: row.room,
...
router.put("/registrations/:registrationId", async (req, res) => {
  ...
  try {
    ...
    const guardianName =
      [guardianFirstName, guardianLastName].filter(Boolean).join(" ") ||
      reg.guardianName;
...
router.patch(
  "/registrations/:registrationId/custom-answers",
  async (req, res) => {
    const registrationId = parseInt(req.params.registrationId, 10);
    if (isNaN(registrationId)) {
      res.status(400).json({ error: "Invalid registrationId" });
        ...
        await db
          ...
          .where(
            and(
              ...
              eq(registrationCustomAnswersTable.registrationId, registrationId),
              eq(
                registrationCustomAnswersTable.organizationId,
                auth.organizationId,
              ),
            ),
          );
...
router.delete("/registrations/:registrationId", async (req, res) => {
  ...
  try {
    ...
    await db
      ...
      .where(eq(answersTable.registrationId, registrationId));
    await db
      .delete(registrationCustomAnswersTable)
      .where(eq(registrationCustomAnswersTable.registrationId, registrationId));
    await db
      .delete(checkinsTable)
      .where(eq(checkinsTable.registrationId, registrationId));
