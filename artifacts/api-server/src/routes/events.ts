...
const TEMPLATE_CHILD_CHECKIN = [
  ...
  {
    fieldKind: "system",
    systemKey: "emergency_contact_name",
  ...
  {
    fieldKind: "system",
    systemKey: "emergency_contact_phone",
...
const TEMPLATE_FAMILY_GROUP = [
  ...
  {
    fieldKind: "system",
    systemKey: "participant_first_name",
    label: "First Name",
    fieldType: "text",
    required: true,
    sortOrder: 0,
  ...
  {
    fieldKind: "system",
    systemKey: "participant_last_name",
    label: "Last Name",
    fieldType: "text",
    required: true,
    sortOrder: 1,
  ...
  {
    fieldKind: "system",
    systemKey: "dietary_restrictions",
...
export async function createEventWithForm(input: CreateEventWithFormInput) {
  ...
  const isChildCheckin =
    !registrationType || registrationType === "child_checkin";
  const resolvedTrackAttendance =
    trackAttendance !== undefined ? trackAttendance : isChildCheckin;
  const resolvedRequireCheckout =
    requireCheckout !== undefined ? requireCheckout : isChildCheckin;
  const resolvedPrintLabels =
    printLabels !== undefined ? printLabels : isChildCheckin;
  const resolvedLabelType =
    labelType || (resolvedRequireCheckout ? "child_security" : "simple_name");
  ...
  if (addDefaultQuestions !== false) {
    ...
    await db
      ...
      .values(template.map((f) => ({ ...f, formId: form.id, organizationId })));
  }
  ...
  if (
    resolvedScheduleType === "repeating" &&
    startDate &&
    endDate &&
    repeatDayOfWeek !== undefined &&
    repeatDayOfWeek !== null
  ) {
    await createEventSessions(
      event.id,
      organizationId,
      startDate,
      endDate,
...
async function buildEventRow(event: typeof eventsTable.$inferSelect) {
  ...
  if (event.formId) {
    const form = await db
      .select()
      .from(formsTable)
      .where(eq(formsTable.id, event.formId))
      .limit(1);
    if (form[0]) {
  ...
  if (event.scheduleType === "repeating") {
    const sessions = await db
      .select({
        ...
        sessionDate: eventSessionsTable.sessionDate,
        status: eventSessionsTable.status,
      })
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!name || !eventType || !formTitle) {
      res
        .status(400)
        .json({ error: "name, eventType, and formTitle are required" });
      return;
    }
...
router.get("/events/:eventId/registrations/export", async (req, res) => {
  ...
  try {
    ...
    for (const c of allCheckins) {
      if (!latestCheckin.has(c.registrationId))
        latestCheckin.set(c.registrationId, c);
    }
    ...
    const allCustomAnswers = await db
      ...
      .orderBy(
        asc(registrationCustomAnswersTable.formFieldId),
        asc(registrationCustomAnswersTable.id),
      );
    ...
    for (const a of allCustomAnswers) {
      if (!answersByReg.has(a.registrationId))
        answersByReg.set(a.registrationId, {});
    ...
    const rows = registrations.map((reg) => {
      ...
      return {
        ...
        fullName: [reg.childFirstName, reg.childLastName]
          .filter(Boolean)
          .join(" "),
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const registration of registrations) {
        ...
        if (
          participantName &&
          !existing.participantNames.includes(participantName)
        ) {
  ...
  try {
    ...
    if (event[0].formId) {
      const formRow = await db
        .select()
        .from(formsTable)
        .where(eq(formsTable.id, event[0].formId))
        .limit(1);
      if (formRow[0]) {
        const [questions, formFields] = await Promise.all([
          db
          ...
          db
            .select()
            .from(formFieldsTable)
            .where(eq(formFieldsTable.formId, formRow[0].id))
            .orderBy(asc(formFieldsTable.sortOrder)),
        ]);
        form = {
          ...formRow[0],
          submissionCount: row.registrationCount,
          questions,
          formFields,
        };
      }
