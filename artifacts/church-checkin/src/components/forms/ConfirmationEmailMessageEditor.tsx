...
function markupToHtml(value: string) {
  return escapeHtml(value)
    .replace(/\[(small|normal|large)\]([\s\S]*?)\[\/\1\]/g, (_match, size: TextSize, content: string) => (
      `<span data-email-size="${size}" class="${SIZE_CLASS[size]}">${content}</span>`
    ))
...
function serializeNode(node: Node): string {
  ...
  const emailSize = node.dataset.emailSize;
  if (emailSize === "small" || emailSize === "normal" || emailSize === "large") {
    return `[${emailSize}]${content}[/${emailSize}]`;
  }
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div
        ...
        className="min-h-[136px] rounded-md border border-input bg-background px-3 py-2 text-sm leading-6 shadow-sm outline-none focus-visible:ring-1 focus-visible:ring-ring"
        onInput={(event) => {
